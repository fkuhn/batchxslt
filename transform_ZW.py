...
pf_corpus = os.path.join(dgd_corpus, 'ZW--_extern.xml')
pf_events = os.path.join(dgd_events, 'ZW')
pf_speakers = os.path.join(dgd_speakers, 'ZW')

xsl_processor = processor.XSLBatchProcessor(saxon_jar)

xsl_processor.transform(corpus_xsl, pf_corpus, "cmdi_", '/tmp/cmdi/corpus/')

xsl_processor.transform(event_xsl, pf_events, "cmdi_", '/tmp/cmdi/events/ZW/')

xsl_processor.transform(speaker_xsl, pf_speakers, "cmdi_", '/tmp/cmdi/speakers/ZW/')
